...
PCA_before <- PCAdata %>% 
  ...
  ggtitle("PCA Analysis, coloring by condition") +
...
PCA_after<- ggplot(data = data.frame(pca$x, samples, samples = samples$condition, stringsAsFactors = F), 
  ...
  theme(plot.title = element_text(lineheight = 0.8, face="bold")) +
  ggtitle("PCA Analysis, coloring by conditon") +
  geom_point(aes(color = condition), size = 3) +
  geom_text_repel(colour = "black", size = 3) +
  geom_hline(yintercept = 0, colour = "lightgrey", size=.1) +
  geom_vline(xintercept = 0, colour = "lightgrey", size=.1) +
...
TraitCor_dropNA<- bloodCellsCor[rowSums(is.na(bloodCellsCor)) !=5,]
# remove Controls
...
##### miRNAs from Timepoint A significantly correlated with Clinical Scores at Time point A
```{r clinA, fig.height= 10}
...
y <- Clin_scoreVars %>% dplyr::select(ends_with("A")) %>% 
  drop_na() %>% 
  as.matrix()
...
## remove all the NAs
TraitCor_dropNA<- moduleTraitCor[rowSums(is.na(moduleTraitCor)) !=4,]
TraitCor_dropCtrls <- TraitCor_dropNA[!grepl("POS", rownames(TraitCor_dropNA)),]
TraitCor_dropCtrls <- TraitCor_dropCtrls[!grepl("NEG", rownames(TraitCor_dropCtrls)),]
TraitCor_dropNA_reorder <- cluster_matrix(TraitCor_dropCtrls)
...
# Display the correlation values within a heatmap plot
labeledHeatmap(Matrix = TraitCorFig,
               xLabels = names(Clin_scoreVars %>% dplyr::select(ends_with("A"))),
               yLabels = rownames(TraitCorFig),
               ySymbols = rownames(TraitCorFig),
               colorLabels = FALSE,
               colors = blueWhiteRed(50),
...
```
**Figure: Clinical Scores at timepoint A, correlated with miRNAs at Timepoint A. Number of observations = `r dim(y)[[1]]`**
...
##### miRNAs from Timepoint A significantly correlated with Clinical Scores at Time point B

```{r clinB, fig.height= 10}
### 1. Clinical Score Variables 


# Spearman’s rank correlation coefficient, or Spearman’s correlation coefficient, as the name suggests, is a nonparametric approach to measuring correlation using rank values.
x <- log2_normA %>% as.matrix()
y <- Clin_scoreVars %>% dplyr::select(ends_with("B")) %>% 
  drop_na() %>% 
  as.matrix()
...
clinBCor = stats::cor(x,
                      y,
                      method = "spearman")# spearman: rank correlation 


## remove all the NAs
TraitCor_dropNA<- clinBCor[rowSums(is.na(clinBCor)) !=4,]
TraitCor_dropCtrls <- TraitCor_dropNA[!grepl("POS", rownames(TraitCor_dropNA)),]
TraitCor_dropCtrls <- TraitCor_dropCtrls[!grepl("NEG", rownames(TraitCor_dropCtrls)),]
TraitCor_dropNA_reorder <- cluster_matrix(TraitCor_dropCtrls)
...
labeledHeatmap(Matrix = TraitCorFig,
               ...
               main = paste("Clinical Score Relationships"))
```
**Figure: Clinical Scores at timepoint B, correlated with miRNAs at Timepoint A. Number of observations = `r dim(y)[[1]]`**
