normfinder <-function(data, group = TRUE, ctVal=FALSE, pStabLim=0.3, sample = "sample", gene = "gene", groups = "group", cq = "cq"){
  ...
  tmp <- data.frame(sample = data %>% pull(sample) %>% as.character(),
                    gene = data %>% pull(gene) %>% as.character(),
                    cq = data %>% pull(cq) %>% as.numeric())
  tmp <- tmp %>%
  ...
  tmp <- data.matrix(tmp[,sample_group %>% pull(sample)])
